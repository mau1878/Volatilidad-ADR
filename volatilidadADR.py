...
@st.cache_data(ttl=300, show_spinner=False)
def fetch_previous_close(ticker, previous_date, ticker_set_option):
  try:
      ticker = ticker.upper()  # Convert to uppercase
      trading_days = get_trading_days(start_date=previous_date - timedelta(days=10), end_date=previous_date, ticker_set=ticker_set_option)
...
def analyze_volatility(intraday_data, previous_close):
  ...
  pos_to_neg = (intraday_data['Cross'] == -1).sum()  # Arriba a Abajo
  neg_to_pos = (intraday_data['Cross'] == 1).sum()   # Abajo a Arriba
  total_crossings = pos_to_neg + neg_to_pos
  return total_crossings, pos_to_neg, neg_to_pos

# Function to analyze last 30 days
def analyze_last_30_days(ticker, end_date, ticker_set_option):
  start_date = end_date - timedelta(days=30)
  trading_days = get_trading_days(start_date=start_date, end_date=end_date, ticker_set=ticker_set_option)
  ...
  for date in selected_trading_days:
      try:
          date_close, _ = fetch_previous_close(ticker, date, ticker_set_option)
          if date_close is None:
              continue  # Si no hay cierre para esta fecha, saltar

          # Encontrar el siguiente día hábil después de 'date'
          next_trading_day = get_last_trading_day(date + timedelta(days=1), ticker_set_option)
          if next_trading_day > date:  # Asegurarse de que el siguiente día sea posterior
              next_close, _ = fetch_previous_close(ticker, next_trading_day, ticker_set_option)
